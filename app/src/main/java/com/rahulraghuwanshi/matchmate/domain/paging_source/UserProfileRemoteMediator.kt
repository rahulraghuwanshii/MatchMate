package com.rahulraghuwanshi.matchmate.domain.paging_source
...
class UserProfileRemoteMediator(
    ...
    override suspend fun initialize(): InitializeAction {
        ...
        return if (System.currentTimeMillis() - (matchMateDatabase.remoteKeyDao().getCreationTime()
                ?: 0) < cacheTimeout
        ) {
     ...
     */
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, UserDetails>): RemoteKeys? {
        // Get the last page that was retrieved, that contained items.
        // From that last page, get the last item
        return state.pages.lastOrNull {
            it.data.isNotEmpty()
        }?.data?.lastOrNull()?.let { userDetails ->
            matchMateDatabase.remoteKeyDao()
                .getRemoteKeyByUserID(userDetails.id.toString())
        }
    }
     ...
     */
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, UserDetails>): RemoteKeys? {
        // Get the first page that was retrieved, that contained items.
        // From that first page, get the first item
        return state.pages.firstOrNull {
            it.data.isNotEmpty()
        }?.data?.firstOrNull()?.let { userDetails ->
            matchMateDatabase.remoteKeyDao()
                .getRemoteKeyByUserID(userDetails.id.toString())
        }
    }
     ...
     */
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, UserDetails>): RemoteKeys? {
        // The paging library is trying to load data after the anchor position
        // Get the item closest to the anchor position
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.let { userDetails ->
                matchMateDatabase.remoteKeyDao()
                    .getRemoteKeyByUserID(userDetails.id.toString())
            }
        }
    ...
    override suspend fun load(
        ...
        try {
            val apiResponse = fetchUserProfileUseCase.invoke(page)
            val userDetailsList = if (apiResponse is ApiResult.Success) {
                apiResponse.data?.userDetails ?: emptyList()
            } else {
                emptyList()
            }

            val endOfPaginationReached = userDetailsList.isEmpty()

            matchMateDatabase.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    matchMateDatabase.remoteKeyDao().clearRemoteKeys()
                    matchMateDatabase.matchMateDao().clearAllUserDetails()
                }
                val prevKey = if (page > 1) page - 1 else null
                val nextKey = if (endOfPaginationReached) null else page + 1
                val remoteKeys = userDetailsList.map {
                    RemoteKeys(
                ...
                matchMateDatabase.remoteKeyDao().insertAll(remoteKeys)
                matchMateDatabase.matchMateDao()
                    .insertAllUser(userDetailsList)
            }
